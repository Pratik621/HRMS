...
const AttendanceReports = () => {
  ...
  // here) and not already Present (nothing to override). Company-wide since this page
  // already lists every employee, not just "my team".
  const earlyLogoutCandidates = activeView === 'daily'
    ? dailyAttendance
        ...
        .map(record => ({
          ...
          clock_in: formatShortTime(record.clock_in),
        }))
    : [];
  ...
  const handleMarkLeft = async (record) => {
    ...
    try {
      ...
      setMessage(err.response?.data?.message || 'Failed to mark employee as left');
      setMessageType('danger');
    } finally {
    ...
    >
          ...
          <div className="d-flex gap-2">
            ...
            {(user?.role === 'admin' || user?.role === 'hr') && (
              <Button
                ...
                onClick={() => setShowEarlyLogout(true)}
                className="text-nowrap rounded-3"
              >
                <FaSignOutAlt className="me-2" size={12} /> Apply Early Logout
              </Button>
            )}
          </div>
        </div>
      ...
      {markLeftConfirm && (
        <ConfirmModal
          ...
          message={`Mark ${`${markLeftConfirm.first_name || ''} ${markLeftConfirm.last_name || ''}`.trim() || markLeftConfirm.employee_id} as having left? Their account stays active and continues as normal until the end of the current salary cycle (the 25th), then automatically deactivates.`}
